...
describe "sign in page", type: :feature, js: true do
  ...
  context "when visit sign in page" do
    ...
    context "when sign in with a existing user" do
      ...
      it "should have the sign out link with valid password" do
        find("#user_password").set(user.password)
        click_button "Sign in to Turing"
        expect(page).to have_link("Sign Out")
      end

      it "should have 'Signed in successfully.' message with valid password" do
        find("#user_password").set(user.password)
        click_button "Sign in to Turing"
