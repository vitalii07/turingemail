...
RSpec.describe AolMailAccount, :type => :model do
  let!(:aol_mail_account) { FactoryGirl.create(:aol_mail_account) }
  let(:original_data) { "original gmail data" }
  let(:encoded_data) { Base64.urlsafe_encode64(original_data) }
  let(:aol_mail_data) { {:raw => encoded_data} }
  ...
  describe "Contants" do
    ...
    describe "::MESSAGE_BATCH_SIZE" do
      it 'returns 100' do
        expect( AolMailAccount::MESSAGE_BATCH_SIZE ).to eq( 100 )
      end
    end

    describe "::DRAFTS_BATCH_SIZE" do
      it 'returns 100' do
        expect( AolMailAccount::DRAFTS_BATCH_SIZE ).to eq( 100 )
      end
    end

    describe "::HISTORY_BATCH_SIZE" do
      it 'returns 100' do
        expect( AolMailAccount::HISTORY_BATCH_SIZE ).to eq( 100 )
      end
    end

    describe "::SEARCH_RESULTS_PER_PAGE" do
      it 'returns 50' do
        expect( AolMailAccount::SEARCH_RESULTS_PER_PAGE ).to eq( 50 )
      end
    end

    describe "::NUM_SYNC_DYNOS" do
      it 'returns 3' do
        expect( AolMailAccount::NUM_SYNC_DYNOS ).to eq( 3 )
      end
    end
    ...
    describe "::SCOPES" do
      it 'returns an empty array' do
        ...
        expect( AolMailAccount::SCOPES ).to eq( expected )
      end
    end

    describe "::SMTP_ADDRESS" do
      it 'returns smtp.mail.me.com' do
        expect( AolMailAccount::SMTP_ADDRESS ).to eq( 'smtp.aol.com' )
      end
    end

    describe "::SYSTEM_FOLDERS" do
      it 'returns the system folders' do
        expect( AolMailAccount::SYSTEM_FOLDERS ).to eq( {"INBOX" => "INBOX",
                                                            "DRAFT" => "Drafts",
                                                             "SENT" => "Sent",
                                                            "TRASH" => "Trash"} )
      end
      ...
      describe "Data Transforming class methods" do
        ...
        describe "#mime_data_from_email_data" do
          ...
          it 'returns the mime data of the email data' do
            expect( AolMailAccount.mime_data_from_email_data(aol_mail_data) ).to eq( original_data )
          end
        ...
        describe "#email_raw_from_email_data" do
          ...
          it 'returns the raw email from the email data' do
            ...
            expect( AolMailAccount.email_raw_from_email_data(aol_mail_data) ).to eq( expected )
          end
        ...
        describe "#init_email_from_email_data" do
          ...
          it 'saves the imap_folder.name + UID of the email data to the uid field of the email' do
            allow(aol_mail_data).to receive(:attr).and_return({ "UID" => uid_value, "FLAGS" => []})

            AolMailAccount.init_email_from_email_data(email, aol_mail_data, imap_folder)
    ...
    describe "Instance methods" do
      ...
      describe "Getter instance methods" do
        ...
        describe '.smtp_address' do
          ...
          it "returns the aol mail account's SMTP_ADDRESS constant value" do
            expect(aol_mail_account.smtp_address).to eq(AolMailAccount::SMTP_ADDRESS)
          end
        ...
        describe '.imap_client' do
          before do
            aol_mail_client_stub = {}
            allow(aol_mail_account).to receive(:aol_mail_client) { aol_mail_client_stub }
          end

          it "returns the aol mail account's aol mail client" do
            expect(aol_mail_account.imap_client).to eq(aol_mail_account.aol_mail_client)
          end

        end

        describe '.aol_mail_client' do
          it "is pending spec implementation"
        end
      ...
      describe "Action instance methods" do
        ...
        describe ".sync_email_folders" do
          let!(:imap_folders) { FactoryGirl.create_list(:imap_folder, 5, email_account: aol_mail_account) }
          let!(:imap_folder_datas) { ["data 1", "data 2", "data 3", "data 4", "data 5"] }

          before do
            allow(aol_mail_account).to receive(:imap_folders_on_server).and_return(imap_folder_datas)
          ...
          it 'syncs the imap folder from the server' do
            aol_mail_account.sync_email_folders
        ...
        describe ".sync_contacts" do
          ...
          it "does nothing" do
            expect(aol_mail_account.sync_contacts('token')).to eq(nil)
          end
        ...
        describe ".sync_account" do
          before(:each) {
            allow(aol_mail_account).to receive(:sync_email_folders)
            allow(aol_mail_account).to receive(:sync_email)

            aol_mail_account.sync_account
          }

          it 'synchronizes the labels' do
            expect(aol_mail_account).to have_received(:sync_email_folders)
          end

          it 'synchronizes the email' do
            expect(aol_mail_account).to have_received(:sync_email)
          end

          it 'updates #last_sync_at attribute' do
            expect(aol_mail_account.last_sync_at).to be_within(1).of Time.now
          end
      ...
      describe "Email Synchronization methods" do
        describe ".sync_email" do
          ...
          describe "when the emails exists on server but not in db" do
            ...
            before do
              allow(aol_mail_account).to receive(:email_uids_in_imap_folder).and_return(email_uids_on_server)
              allow(aol_mail_account).to receive(:threads_in_imap_folder).and_return([])
              allow(aol_mail_account).to receive(:email_data_for_email_uids).and_return(email_uids_on_server_and_not_in_db)
              allow(aol_mail_account).to receive(:create_email_from_email_data).and_return(emails)
              allow(aol_mail_account).to receive(:get_thread_id).and_return(emails.first.uid)
            end

            it 'creates the emails in db' do
              aol_mail_account.sync_email
              expect(aol_mail_account).to have_received(:create_email_from_email_data).exactly(10).times
            end
          end
          ...
          describe "when the emails exists in db but not on server" do
            ...
            before do
              ...
              allow(aol_mail_account).to receive(:email_uids_in_imap_folder).and_return([])
              allow(aol_mail_account).to receive(:threads_in_imap_folder).and_return([])
